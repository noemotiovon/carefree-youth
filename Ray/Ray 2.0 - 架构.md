...
GCS 在 actor 管理中扮演着重要角色。所有的 actor 必须首先在 GCS 中注册，才能被调度。GCS 还是分离（detached）actor 的所有者。有关更多信息，请参见 actor 管理部分。
...
作业提交在 Ray 2.0 中处于 beta 阶段。退出 beta 阶段的关键步骤之一是使作业监督器 actors 能够在主节点以外的节点上调度，以减少在多租户设置中对主节点的压力。

## 运行时环境和多租户

运行时环境定义了执行 Python 脚本所需的依赖项，如文件、包和环境变量。它在运行时动态安装到集群中，可以为 Ray 作业、特定的 actor 或 task 指定运行时环境。

运行时环境的安装和删除由运行在每个节点上的 RuntimeEnvAgent gRPC 服务器处理。RuntimeEnvAgent 与 raylet 命运共享，以简化故障模型，并且它是调度任务和 actors 的核心组件。

当 task 或 actor 需要运行时环境时，raylet 会向 RuntimeEnvAgent 发送 gRPC 请求，若环境尚不存在，则创建该环境。

![img](./images/36_Ray2-34.png)

## KubeRay

KubeRay 操作符在 Kubernetes 环境中管理 Ray 集群。每个 Ray 节点作为一个 Kubernetes pod 运行。KubeRay 遵循 Kubernetes 操作符模式：

- 一个名为 *RayCluster* 的自定义资源描述了 Ray 集群的期望状态。
- 一个名为 *KubeRay 操作符* 的自定义控制器管理 Ray pods，以使其符合 RayCluster 的规范。

有关 KubeRay 的更多信息，请参见 [文档](https://docs.ray.io/en/latest/cluster/kubernetes/index.html)。

## Ray 可观察性

Ray 提供了多种[工具](https://docs.ray.io/en/master/ray-observability/index.html)和功能，以提高对集群的可视性。

### Ray 仪表盘

Ray 提供了一个内置的仪表盘，它作为一个 HTTP 服务器在主节点上运行。Ray 仪表盘定期从集群聚合系统状态，组织和存储数据，并提供 Web UI 来可视化集群状态。

### 日志聚合

Ray driver 聚合并打印所有来自 actors 和 tasks 的日志消息。当 task 或 actor 将日志打印到其 stdout 或 stderr 时，这些日志会自动重定向到相应的 worker 日志文件。一个名为日志监视器的 Python 进程在每个节点上运行。日志监视器定期读取本地 Ray 日志文件，并通过 GCS pubsub 将日志消息发布到 driver 程序。

![img](./images/36_Ray2-35.png)

### 指标

Ray 原生集成了 OpenCensus，并默认支持导出到 Prometheus。所有 Ray 组件（GCS 服务、Raylet、Workers）将指标发送到它们本地的 Ray agent 进程。每个 Ray agent 通过 OpenCensus（默认作为 Prometheus 端点）公开这些指标。

![img](./images/36_Ray2-36.png)

### Ray 状态 API

自 Ray 2.0 起，Ray 支持状态 API，允许用户通过 CLI 或 Python SDK 方便地访问当前 Ray 状态的快照。状态 API 支持对特定 Ray tasks、actors 等的摘要和查询。

像 Kubernetes 这样的系统，支持类似的 API，通常会将执行元数据（例如当前的 pods）写入持久化的键值存储或数据库。相比之下，Ray 不会持久化执行信息，如当前正在运行的 tasks。这是因为 Ray 的 tasks、对象等更加轻量，写入此类元数据到数据库的成本将极高。相反，执行元数据通过所有权模型分布在 workers 中，并与 workers 命运共享。因此，要执行如“列出所有任务”这样的查询，状态 API 必须按需从数据源（例如 workers）查询数据。
...
e. 大对象的存储和检索。检索通过 `ray.get` 或在任务执行期间进行，当替换任务的 ObjectID 参数为对象的值时。

f. 调度器从远程节点获取对象，以满足本地排队任务的依赖关系。

## 任务调度和对象存储示例

我们将通过以下 Ray 程序的物理执行过程来进行讲解：
...
在这个示例中，任务 A 提交了任务 B 和任务 C，并且 C 依赖于 B 的输出。为了说明这一点，假设 B 返回一个大对象 X，C 返回一个小对象 Y。这样，我们就可以展示进程内和共享内存对象存储之间的区别。我们还将展示如果任务 A、B 和 C 都在不同的节点上执行时，分布式调度是如何工作的。

### 分布式任务调度

首先，假设 worker 1 执行任务 A。任务 B 和 C 已经被提交到 worker 1。因此，worker 1 的本地所有权表已经包括 X 和 Y 的条目。接下来，我们将通过一个示例，讲解如何调度任务 B 执行：

![img](./images/36_Ray2-38.png)

1. Worker 1 向其本地调度器请求资源以执行任务 B。
2. 调度器 1 响应，告诉 worker 1 在节点 2 上重新尝试调度请求。
3. Worker 1 更新其本地所有权表，标记任务 B 正在节点 2 上等待执行。
4. Worker 1 向节点 2 上的调度器请求资源以执行任务 B。
5. 调度器 2 分配资源给 worker 1，并响应 worker 2 的地址。调度器 2 确保在 worker 1 持有资源时，不会将其他任务分配给 worker 2。
6. Worker 1 将任务 B 发送到 worker 2 执行。

### 任务执行

接下来，我们将展示一个 worker 执行任务并将返回值存储到分布式对象存储中的示例：

![img](./images/36_Ray2-39.png)

1. Worker 2 完成执行任务 B，并将返回值 X 存储到其本地对象存储中。

   A. 节点 2 异步更新对象表，指示 X 现在位于节点 2 上（虚线箭头）。

   B. 由于这是 X 的第一个副本，节点 2 还会固定它的副本，直到 worker 1 通知节点 2 可以释放该对象（未显示）。这确保了在对象仍被引用时，其值是可达的。

2. Worker 2 向 worker 1 响应，表示任务 B 已完成。

3. Worker 1 更新其本地所有权表，标记 X 存储在分布式内存中。

4. Worker 1 将资源返还给调度器 2。Worker 2 现在可以被重用来执行其他任务。

### 分布式任务调度和参数解析

现在任务 B 已经完成，任务 C 可以开始执行。Worker 1 接下来调度任务 C，使用与任务 B 相似的协议：

![img](./images/36_Ray2-40.png)

1. Worker 1 向其本地调度器请求资源以执行任务 C。
2. 调度器 1 响应，告诉 worker 1 在节点 3 上重新尝试调度请求。
3. Worker 1 更新其本地所有权表，标记任务 C 正在节点 3 上等待执行。
4. Worker 1 向节点 3 上的调度器请求资源以执行任务 C。
5. 调度器 3 发现任务 C 依赖于 X，但它的本地对象存储中没有 X 的副本。调度器 3 将任务 C 排队，并向对象表请求 X 的位置。

任务 C 需要 X 的本地副本才能开始执行，因此节点 3 获取 X 的副本：

![img](./images/36_Ray2-41.png)
...
3. X 从节点 2 复制到节点 3。
   ...
   A. 节点 3 也异步更新对象表，指示 X 现在也位于节点 3（虚线箭头）。

   B. 节点 3 的 X 副本被缓存，但未被固定。虽然本地 worker 正在使用它，但该对象不会被逐出。然而，与节点 2 上的 X 副本不同，节点 3 上的副本可能会在对象存储 3 遭遇内存压力时根据 LRU 被逐出。如果发生这种情况，并且节点 3 以后再次需要该对象，它可以按照相同的协议从节点 2 或其他副本重新获取该对象。

4. 由于节点 3 现在有了 X 的本地副本，调度器 3 将资源分配给 worker 1，并响应 worker 3 的地址。

### 任务执行和对象内联

任务 C 执行并返回一个足够小的对象，可以存储在进程内内存存储中：

![img](./images/36_Ray2-42.png)

1. Worker 1 将任务 C 发送到 worker 3 执行。
2. Worker 3 从其本地对象存储中获取 X 的值（类似于 `ray.get`），并运行 C(X)。
...
![img](./images/36_Ray2-43.png)

1. Worker 1 删除了其对象 X 的条目。这是安全的，因为待处理的任务 C 是 X 唯一的引用，而 C 已经完成。Worker 1 保留了 Y 的条目，因为应用程序仍然持有 Y 的 ObjectID 引用。

   A. 最终，X 的所有副本都会从集群中删除。这可以在步骤 1 之后的任何时刻进行。如上所述，如果节点 3 的对象存储遭遇内存压力，节点 3 上的 X 副本也可能在步骤 1 之前被删除。
